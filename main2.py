...
def build_feature_combos_by_sum(min_sum=7, max_sum=64, use_uv_raw=False):
    """
    Enumerate feature index combos. Assumes final X_full will be 64-dim:
      0..56 base floats
      57 ray_normal_dot
      58 determinant
      59 u_raw (selectable only if use_uv_raw=True)
      60 v_raw (selectable only if use_uv_raw=True)
      61 dot_v0
      62 dot_v1
      63 dot_v2
    ...
    Args:
        min_sum: Minimum feature dimension to include
        max_sum: Maximum feature dimension to include
        use_uv_raw: If True, include u_raw (idx 59) and v_raw (idx 60) in all combos
    """
    FEATURE_SLICES = {
        ...
        "determinant":        [58],
        "u_raw":              [59],  # selectable only when use_uv_raw=True
        "v_raw":              [60],  # selectable only when use_uv_raw=True
        "dot_v0":             [61],
        "dot_v1":             [62],
        "dot_v2":             [63],
    }
    ...
    for mask in range(1 << n_base):
        ...
        for a in [None] + groupA:
            ...
            for b in [None] + groupB:
                ...
                # Conditionally add u_raw and v_raw if enabled
                if use_uv_raw:
                    final_indices = sorted(set(final_indices + [59, 60]))  # u_raw=59, v_raw=60
                    final_features = sorted(set(final_features + ["u_raw", "v_raw"]))
...
def build_full_X_and_y(X_base_57, y_raw_u8, scale_ray_dirs=True):
    ...
    if scale_ray_dirs:
        centroid = X[:, 24:27]  # Centroid coordinates from base features
        lengths = np.linalg.norm(centroid, axis=1, keepdims=True).astype(np.float32)
        # FIX: Prevent zeroing out ray direction when centroid is at origin
        lengths = np.where(lengths > 1e-12, lengths, 1.0)
        X[:, 0:3] *= lengths
        print("Scaled ray directions using triangle centroid distance to origin (model sees scaled ray_dir).")
    else:
...
def save_topk_checkpoints(top_k_models, checkpoint_dir):
    ...
    for rank, m in enumerate(top_k_models, start=1):
        ...
        payload = {
            ...
            "use_uv_raw": m.get("use_uv_raw", False),
...
if __name__ == "__main__":
    ...
    DATA_FILE = os.path.join(SCRIPT_DIR, "triangles.bin")
    NAS_FILE = os.path.join(SCRIPT_DIR, "nas.txt")
    ...
    SCALE_RAY_DIR = True
    USE_UV_RAW = False  # If True, include u_raw and v_raw features in training

    # Cost constants for Möller-Trumbore derived features
    # These represent the computational cost of calculating each feature at runtime.
    # u_raw and v_raw are selectable when USE_UV_RAW=True.
    MT_FEATURE_COSTS = {
        "determinant": 5,
        "u_raw": 3,
        "v_raw": 3,
    ...
    # ---- Build features ----
    combos_by_sum = build_feature_combos_by_sum(min_sum=7, max_sum=64, use_uv_raw=USE_UV_RAW)
    validate_feature_combos(combos_by_sum, expected_dim=64)
    
    if USE_UV_RAW:
        print("USE_UV_RAW=True: u_raw and v_raw features (indices 59, 60) are INCLUDED in training.")
    else:
        print("USE_UV_RAW=False: u_raw and v_raw features are EXCLUDED from training.")
    ...
    if hit_count > 0:
        pos_weight_value = float(miss_count / hit_count)
        print(f"Using pos_weight = {pos_weight_value:.4f} (miss/hit ratio)")
    else:
    ...
    # Ensure graph directory exists (re-create if somehow removed during training)
    if not os.path.exists(GRAPH_DIR):
        print(f"  Creating graph directory: {GRAPH_DIR}")
        os.makedirs(GRAPH_DIR, exist_ok=True)
    ...
    for i, m in enumerate(evaluated_topk, start=1):
        try:
            ...
            save_path = os.path.join(GRAPH_DIR, fname)
    ...
    # ---- Log file ----
    report_path = os.path.join(SCRIPT_DIR, "nas_exhaustive_report.txt")
    log_results_to_file(
        results=results,
