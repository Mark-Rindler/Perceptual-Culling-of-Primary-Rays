...
def sample_triangles_no_duplicates(
...
) -> List[Tuple[np.ndarray, np.ndarray, Triangle]]:
    """
    ...
    Returns list of (ray_origin, ray_dir, triangle).
    """
    if seed is not None:
        random.seed(seed)
        np.random.seed(seed)
    ...
    results: List[Tuple[np.ndarray, np.ndarray, Triangle]] = []
    total_retries = 0
    ...
    while len(results) < n_triangles:
        ...
        if retries >= max_retries_per_sample:
            ...
            results.append((leaf.ray_origin.copy(), leaf.ray_dir.copy(), tri))
...
def process_and_write_binary(samples: List[LeafSample],
    ...
    for ray_origin, ray_dir, tri in sampled:
        # Center coordinates by subtracting ray origin
        v0_centered = tri.v0 - ray_origin
        v1_centered = tri.v1 - ray_origin
        v2_centered = tri.v2 - ray_origin

        # Compute hit/miss (ray origin now at 0,0,0)
        ...
        features = build_feature_vector(ray_dir, v0_centered, v1_centered, v2_centered)
...
if __name__ == "__main__":
    ...
    if args.all:
        ...
        for sample in samples:
            for leaf in sample.leaves:
                for tri in leaf.triangles:
                    all_data.append((leaf.ray_origin, leaf.ray_dir, tri))
        ...
        for ray_origin, ray_dir, tri in all_data:
            v0_centered = tri.v0 - ray_origin
            v1_centered = tri.v1 - ray_origin
            v2_centered = tri.v2 - ray_origin
